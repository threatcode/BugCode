...
from bugcode_plugins import __version__
from bugcode_plugins.plugins.manager import PluginsManager, ReportAnalyzer, CommandAnalyzer
from bugcode_plugins.plugins.plugin import PluginByExtension

root_logger = logging.getLogger("bugcode")
...
def list_plugins(custom_plugins_folder):
    ...
    click.echo(click.style(f"Bugcode Plugins v{__version__}", fg="cyan"))
...
def detect_report(report_file, custom_plugins_folder):
    if not os.path.isfile(report_file):
        ...
        plugin = analyzer.get_plugin(report_file)
        if plugin:
            click.echo(click.style(f"Bugcode Plugin: {plugin.id}", fg="cyan"))
        else:
            click.echo(click.style(f"Failed to detect report: {report_file}", fg="red"), err=True)
