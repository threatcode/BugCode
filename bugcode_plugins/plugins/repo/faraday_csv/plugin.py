...
Copyright (C) 2013  KhulnaSoft Ltd (https://www.threatcode.github.io/bugcode/)
...
class CSVParser:
    ...
    def check_objects_to_import(self, headers):
        ...
        # From valid_headers, Bugcode will define which objects to import
        valid_headers = [
            "ip",
            "port", "protocol",
            "name", "desc", "target"
    ...
    def build_hostnames_list(self, row):
        ...
        if "hostnames" in row:
            try:
                ...
                self.logger.error("Hostname not valid. Bugcode will set it as empty.")
        return hostnames
...
class BugcodeCSVPlugin(PluginCSVFormat):

    def __init__(self, *arg, **kwargs):
        super().__init__(*arg, **kwargs)
        self.id = "bugcode_csv"
        self.name = "Bugcode CSV Plugin"
...
def createPlugin(*args, **kwargs):
    return BugcodeCSVPlugin(*args, **kwargs)
