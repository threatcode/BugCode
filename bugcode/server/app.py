...
def setup_storage_path():
    default_path = CONST_BUGCODE_HOME_PATH / 'storage'
...
def create_app(db_connection_string=None, testing=None, register_extensions_flag=True):
    ...
    app.config['SQLALCHEMY_ECHO'] = 'BUGCODE_LOG_QUERY' in os.environ
...
def get_app(db_connection_string=None, testing=None, register_extensions_flag=True):
    global BUGCODE_APP  # pylint: disable=W0603
    if not BUGCODE_APP:
        BUGCODE_APP = create_app(db_connection_string=db_connection_string,
                                 testing=testing,
