...
def start_server():
    ...
    WORKDIR = BUGCODE_BASE  # pylint:disable=unused-variable
    createDaemon()
...
def get_server_pid(port):
    if not Path(str(BUGCODE_SERVER_PID_FILE).format(port)).exists():
        return None

    with open(str(BUGCODE_SERVER_PID_FILE).format(port), encoding='utf-8') as pid_file:
...
def create_pid_file(port):
    with open(str(BUGCODE_SERVER_PID_FILE).format(port), 'w', encoding='utf-8') as pid_file:
        pid_file.write(f'{os.getpid()}')
    atexit.register(partial(remove_pid_file, port))


def remove_pid_file(port):
    os.remove(str(BUGCODE_SERVER_PID_FILE).format(port))
