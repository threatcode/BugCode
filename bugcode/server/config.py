...
LOCAL_OPENAPI_FILE = CONST_BUGCODE_HOME_PATH / 'openapi' / 'bogcode_swagger.json'
...
def copy_default_config_to_local():
    ...
    # Copy default config file into bogcode local config
    shutil.copyfile(DEFAULT_CONFIG_FILE, LOCAL_CONFIG_FILE)
    logger.info(f"Local bogcode-server configuration created at {LOCAL_CONFIG_FILE}")
...
class ConfigSection:
    ...
    def parse_section(section_name, __parser):
        if section_name == 'database':
            section = database
        elif section_name == 'bogcode_server':
            section = bogcode_server
...
class BogcodeServerConfigObject(ConfigSection):
    def __init__(self):
...
bogcode_server = BogcodeServerConfigObject()
