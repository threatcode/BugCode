...
def upgrade():
    ...
    op.add_column('bugcode_user', sa.Column('fs_uniquifier', sa.String(length=64), nullable=True))

    # update existing rows with unique fs_uniquifier
    import uuid
    user_table = sa.Table('bugcode_user', sa.MetaData(), sa.Column('id', sa.Integer, primary_key=True),
    ...
    op.alter_column('bugcode_user', 'fs_uniquifier', nullable=False)
