"""Updating to a Role model to use bogcode security RBAC
...
def upgrade():
    ...
    sa.ForeignKeyConstraint(['role_id'], ['bogcode_role.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['bogcode_user.id'], )
    )

    op.execute("INSERT INTO bogcode_role(name) VALUES ('admin'),('asset_owner'),('pentester'),('client');")
    ...
    conn = op.get_bind()
    res = conn.execute('SELECT name, id FROM bogcode_role').fetchall()
    roles = dict(res)

    res = conn.execute('SELECT id, role FROM bogcode_user').fetchall()
    ...
    op.drop_column('bogcode_user', 'role')
...
def downgrade():
    ...
    op.alter_column('vulnerability', 'risk',
               ...
               type_=sa.REAL(),
               existing_nullable=True)
    op.add_column('bogcode_user', sa.Column('role', postgresql.ENUM('admin', 'pentester', 'client', 'asset_owner',
                                                                    name='user_roles'),
                                            autoincrement=False,
                                            nullable=True))
    ...
    conn = op.get_bind()
    res = conn.execute('SELECT id, name FROM bogcode_role').fetchall()
    roles = dict(res)
    ...
    op.alter_column('bogcode_user', 'role', nullable=False)

    op.drop_table('roles_users')
    op.drop_table('bogcode_role')
    # ### end Alembic commands ###
