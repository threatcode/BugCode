...
def upgrade():
    ...
    op.execute('ALTER TABLE bogcode_user ALTER COLUMN role TYPE _user_roles'
               ' USING role::text::_user_roles')
    old_type.drop(op.get_bind(), checkfirst=False)
    # Create and convert to the "new" status type
    new_type.create(op.get_bind(), checkfirst=False)
    op.execute('ALTER TABLE bogcode_user ALTER COLUMN role TYPE user_roles'
...
def downgrade():
    ...
    tmp_type.create(op.get_bind(), checkfirst=False)
    op.execute('ALTER TABLE bogcode_user ALTER COLUMN status TYPE _user_roles'
               ' USING role::text::_user_roles')
