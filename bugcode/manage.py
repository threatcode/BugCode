...
from bogcode.server.commands.bogcode_schema_display import DatabaseSchema
from bogcode.server.commands.app_urls import show_all_urls
from bogcode.server.commands.app_urls import openapi_format
from bogcode.server.commands import change_password as change_pass
from bogcode.server.commands.custom_fields import add_custom_field_main, delete_custom_field_main
from bogcode.server.commands import change_username
from bogcode.server.commands import nginx_config
from bogcode.server.commands import import_vulnerability_template
from bogcode.server.commands import manage_settings
from bogcode.server.models import db, User, LOCAL_TYPE
from bogcode_plugins.plugins.manager import PluginsManager
from bogcode.server.commands.move_references import _move_references


CONTEXT_SETTINGS = {'help_option_names': ['-h', '--help']}

os.environ['BUGCODE_MANAGE_RUNNING'] = "1"
# If is linux and its installed with deb or rpm, it must run with a user in the bogcode group
if platform.system() == "Linux":
    import grp
    from getpass import getuser

    try:
        BUGCODE_GROUP = "bogcode"
        bogcode_group = grp.getgrnam(BUGCODE_GROUP)
        # The current user may be different from the logged user
        current_user = getuser()
        if current_user != 'root' and bogcode_group.gr_gid not in os.getgroups():
...
def check_bogcode_server(url):
    return requests.get(url, timeout=10)


@click.command(help="Show all URLs in Bogcode Server API")
def show_urls():
    with app.app_context():
        show_all_urls()


@click.command(help="Creates Bogcode Swagger config file")
...
@click.command(help="Create Bogcode DB in Postgresql, also tables and indexes")
@click.option(
    '--choose-password', is_flag=True, default=False,
    help=('Instead of using a random password for the user "bogcode", '
...
def initdb(choose_password, password):
    with app.app_context():
        InitDB().run(choose_password=choose_password, bogcode_user_password=password)


@click.command(help="Create a PNG image with Bogcode model object")
def database_schema():
    DatabaseSchema().run()


@click.command(help="Open a SQL Shell connected to postgresql 'Bogcode DB'")
def sql_shell():
    conn_string = bogcode.server.config.database.connection_string.strip("'")
...
@click.command(help="Create ADMIN user for Bogcode application")
        ...
        if not conn_string:
            ...
            logger.error(
                'No database configuration found. Please check: '
                 ...
                 'For first time installations execute: bogcode-manage initdb'
            )
            sys.exit(1)
...
@click.option('--fqdn', prompt='Server FQDN', help='The FQDN of your bogcode server', type=str, show_default=True)
@click.option('--port', prompt='Bogcode port', help='Bogcode listening port', type=int, default=5985)
@click.option('--ws-port', prompt='Bogcode Websocket port', help='Bogcode websocket listening port', type=int,
