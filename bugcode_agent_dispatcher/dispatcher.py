...
# Copyright (C) 2019  ThreatCode (http://www.threatcode.github.io/bugcode/)
...
import click
from bugcode_agent_dispatcher.config import reset_config
from bugcode_agent_dispatcher.executor_helper import (
    StdErrLineProcessor,
    StdOutLineProcessor,
)
from bugcode_agent_dispatcher.utils.control_values_utils import (
    control_registration_token,
)
from bugcode_agent_dispatcher.utils.text_utils import Bcolors
from bugcode_agent_dispatcher.utils.url_utils import api_url, websocket_url
import bugcode_agent_dispatcher.logger as logging
from bugcode_agent_dispatcher import config
from bugcode_agent_dispatcher.config import (
    Sections,
    save_config,
    control_config,
)
from bugcode_agent_dispatcher.utils.metadata_utils import (
    executor_metadata,
    check_metadata,
)
from bugcode_agent_dispatcher.cli.utils.model_load import set_repo_params
from bugcode_agent_dispatcher.executor import Executor
from bugcode_agent_parameters_types.utils import type_validate
            ...
            try:
                ...
                if e.status == 404:
                    ...
                    logger.error(
                        ...
                        "`bugcode-dispatcher config-wizard`\nHint: "
                        "If the bugcode version is not the expected this"
                        "could fail, check "
                        "https://github.com/threatcode/bugcode_agent_dispatcher"
        ...
        try:
            ...
            if e.status == 402:
                ...
                error_msg = (
                    ...
                    "token with the wizard command `bugcode-dispatcher "
                    "config-wizard`"
                )
    ...
    async def connect(self):
        ...
        async with websockets.connect(
            ...
            **self.ws_kwargs,
            ...
            logger.info("Connection to Bugcode server succeeded")
    ...
    async def check_connection(self):
        ...
        try:
            ...
            print(
                ...
                "`bugcode-dispatcher config-wizard` and check the "
                "certificate configuration"
            )
            return False
        except ClientConnectorError as e:
            logger.error("Can not connect to Bugcode server")
            logger.debug("Connect failed traceback", exc_info=e)
            return False
        except asyncio.TimeoutError as e:
            logger.error("Bugcode server timed-out. " "TIP: Check ssl configuration")
