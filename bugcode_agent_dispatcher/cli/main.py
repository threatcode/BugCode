...
"""Console script for bugcode_agent_dispatcher."""
import os
import signal
import sys
...
from bugcode_agent_dispatcher.cli.wizard import Wizard, DEFAULT_PAGE_SIZE
from bugcode_agent_dispatcher.dispatcher_io import Dispatcher, sio, DispatcherNamespace
from bugcode_agent_dispatcher import config, __version__
...
def process_config_file(config_filepath: Path, logger):
    if config_filepath is None and not os.path.exists(config.CONFIG_FILENAME):
        logger.info("Config file doesn't exist. Run the command " "`bugcode-dispatcher config-wizard` to create one")
        exit(1)
...
@click.command(help="bugcode-dispatcher run")
...
@click.option("--token", help="Registration token from Bugcode server")
...
@click.command(help="bugcode-dispatcher config_wizard")
