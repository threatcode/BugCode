...
from bugcode_agent_parameters_types.utils import get_manifests
from bugcode_agent_dispatcher import __version__ as current_version
import os
import logging
...
try:
    BUGCODE_PATH = Path(os.environ["BUGCODE_HOME"]).expanduser()
except KeyError:
    BUGCODE_PATH = Path("~").expanduser() / ".bugcode"


LOGS_PATH = BUGCODE_PATH / "logs"
CONFIG_PATH = BUGCODE_PATH / "config"


if not BUGCODE_PATH.exists():
            ...
            if (
                ...
                if len(new_repo_name) == 0:
                    logging.warning(
                        ...
                        f"to delete and add it by bugcode-dispatcher"
