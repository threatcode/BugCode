...
from bugcode_plugins.plugins.repo.appscan.plugin import AppScanPlugin
...
def get_report_name():
    return f"{datetime.datetime.now().timestamp()}-bugcode-agent"
...
def create_and_execute_dast_scan(token, target_url, app_id, scan_name):
    body = {
        ...
        "AppId": app_id,
        "Execute": True,
        "Comment": "Scan Created from bugcode",
    }
    headers = {"Authorization": f"Bearer {token}"}
